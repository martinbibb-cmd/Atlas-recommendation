...
function buildFixture() {
  ...
  return { implementationPack, handover };
}
...
describe('buildEngineerJobPack', () => {
  it('output contains no educational copy', () => {
    const { implementationPack, handover } = buildFixture();
    const pack = buildEngineerJobPack(handover, implementationPack);
  ...
  it('enforces max 7 bullets per section', () => {
    ...
    const pack = buildEngineerJobPack(handover, inflatedImplementationPack);
    for (const section of allSections(pack)) {
  ...
  it('is deterministic for the same inputs', () => {
    const { implementationPack, handover } = buildFixture();
    const first = buildEngineerJobPack(handover, implementationPack);
    const second = buildEngineerJobPack(handover, implementationPack);
