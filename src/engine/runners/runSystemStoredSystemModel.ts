/**
 * runSystemStoredSystemModel.ts — PR2: Topology-aware runner for system boiler
 * (sealed-system) installations.
 *
 * This runner owns the stored-water module calls for sealed-system boiler
 * configurations (e.g. a Viessmann Vitodens or Worcester Greenstar with an
 * unvented cylinder).  It operates without a direct draw-off path:
 * the topology has `drawOff === undefined`.
 *
 * Owned modules:
 *   - runStoredDhwModuleV1       (stored DHW capacity, cylinder sizing, simultaneous demand)
 ...
 *
 * Common modules (also called in other runners):
 ...
 * The `SystemTopology` passed in must have `appliance.family === 'system'`
 ...
 * `appliance.family === 'system'` and `drawOff === undefined`.
 *
 * @param input    Raw engine input from the survey layer.
 * @param topology PR1 topology contract — must be a system boiler topology.
 */
export function runSystemStoredSystemModel(
  ...
  return {
    ...
    dhw: {
      // PR3: canonical DHW envelope — system runner owns storedDhwV1, mixergy, mixergyLegacy.
