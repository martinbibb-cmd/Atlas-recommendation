...
describe('runCwsSupplyModuleV1', () => {
  ...
  it('deliveryMode pumped_from_tank → same note as pumped (canonical)', () => {
    const result = runCwsSupplyModuleV1(
      baseInput({
        dynamicMainsPressure: 1.5,
        mainsDynamicFlowLpm: 9,
  ...
  it('deliveryMode accumulator_supported → notes mention accumulator and mains supply', () => {
    ...
    expect(result.notes.some(n => n.toLowerCase().includes('accumulator'))).toBe(true);
    expect(result.notes.some(n => n.toLowerCase().includes('mains supply'))).toBe(true);
  });

  // ── deliveryMode break_tank_booster ──────────────────────────────────────────

  it('deliveryMode break_tank_booster → notes mention break tank and stored water', () => {
    const result = runCwsSupplyModuleV1(
      baseInput({
        dynamicMainsPressure: 2.0,
        mainsDynamicFlowLpm: 12,
    ...
    expect(result.notes.some(n => n.toLowerCase().includes('break tank'))).toBe(true);
    expect(result.notes.some(n => n.toLowerCase().includes('stored water'))).toBe(true);
  });
