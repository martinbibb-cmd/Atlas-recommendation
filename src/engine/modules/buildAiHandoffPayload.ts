...
// ─── Locked summary serialiser ────────────────────────────────────────────────

/**
 * buildLockedAiHandoffText
 *
 * Serialises a CustomerSummaryV1 into a visible, copyable plain-text block
 * for use in the customer print pack AI handoff section.
 *
 * Rules:
 *   - Case-specific content is derived from CustomerSummaryV1 only.
 *   - Does not include rejected scenarios, ranked options, viability notes,
 *     or raw survey context.
 *   - Labelled as explanation/export, not recommendation.
 *   - Static policy fields (introduction, validation policy, source rules)
 *     are included so external AI assistants handle the payload responsibly.
 */
export function buildLockedAiHandoffText(summary: CustomerSummaryV1): string {
  const lines: string[] = [
    '=== ATLAS RECOMMENDATION SUMMARY ===',
    '',
    '--- Instructions for the AI assistant ---',
    ...
    '',
    'Trusted source categories:',
    ...TRUSTED_SOURCE_CATEGORIES.map((c) => `• ${c}`),
    '',
    'Source use rules:',
    ...SOURCE_USE_RULES.map((r) => `• ${r}`),
    '',
    '--- Case-specific Atlas data ---',
    '',
    `Recommended: ${summary.recommendedSystemLabel} — ${summary.headline}`,
    '',
  ];
  ...
  if (summary.whatThisAvoids.length > 0) {
    lines.push('What this avoids:');
    summary.whatThisAvoids.forEach((r) => lines.push(`• ${r}`));
    lines.push('');
  }

  if (summary.includedNow.length > 0) {
    lines.push('Included scope:');
    summary.includedNow.forEach((i) => lines.push(`• ${i}`));
    lines.push('');
  }

  if (summary.requiredChecks.length > 0) {
    lines.push('Required checks:');
    summary.requiredChecks.forEach((c) => lines.push(`• ${c}`));
    lines.push('');
  }

  if (summary.optionalUpgrades.length > 0) {
    lines.push('Recommended upgrades (advised but not yet committed):');
    summary.optionalUpgrades.forEach((u) => lines.push(`• ${u}`));
    lines.push('');
  }

  if (summary.futureReady.length > 0) {
    lines.push('Future upgrades:');
    summary.futureReady.forEach((p) => lines.push(`• ${p}`));
    lines.push('');
  }

  if (summary.confidenceNotes.length > 0) {
    lines.push('Confidence notes:');
    summary.confidenceNotes.forEach((n) => lines.push(`• ${n}`));
    lines.push('');
  }
