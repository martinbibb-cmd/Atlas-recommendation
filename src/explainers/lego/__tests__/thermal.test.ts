...
import { computeTmvMixer } from '../animation/thermal'

describe('computeTmvMixer — basic physics', () => {
  ...
  it('returns a TmvOutcome with all required fields', () => {
    ...
    expect(typeof r.T_mix).toBe('number')
    expect(typeof r.saturated).toBe('boolean')
    expect(typeof r.shortfallC).toBe('number')
  })
...
describe('computeTmvMixer — saturation', () => {
  it('saturates when shower flow is too high for the boiler to maintain target', () => {
    ...
    expect(r.T_mix).toBeLessThan(40)  // delivered temp below target
    expect(r.shortfallC).toBeGreaterThan(0)
  })
