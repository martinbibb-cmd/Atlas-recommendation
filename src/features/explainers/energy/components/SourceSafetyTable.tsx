/**
 * SourceSafetyTable.tsx
 *
 * Data-driven table: deaths per TWh by energy source.
...
import { formatDeathsPerTwh } from '../lib/energyFormatting';
import { ENERGY_COPY } from '../data/energyExplainerCopy';
import './SourceTable.css';

export default function SourceSafetyTable() {
  const rows = ENERGY_SOURCE_FACTS.filter(
    (f) => f.typicalDeathsPerTwh != null,
  ).sort((a, b) => (a.typicalDeathsPerTwh ?? 0) - (b.typicalDeathsPerTwh ?? 0));

  return (
    <EnergyExplainerCard
      title={ENERGY_COPY.sourceTable.safetyTitle}
      badge="Data"
      className="src-table"
    >
      <p className="src-table__subtitle">{ENERGY_COPY.sourceTable.safetySubtitle}</p>
        ...
        <table className="src-table__table">
          <thead>
            <tr>
              ...
              <th scope="col" className="src-table__th src-table__th--number">Deaths / TWh</th>
            ...
            {rows.map((fact) => (
              <tr key={fact.id} className="src-table__tr">
                ...
                <td className="src-table__td src-table__td--number">
                  {formatDeathsPerTwh(fact.typicalDeathsPerTwh!)}
                </td>
