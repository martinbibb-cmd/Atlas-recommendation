/**
 * buildInstallationSpecificationDraft.test.ts
 *
 * Unit tests for buildInstallationSpecificationDraft.
 *
...
import { buildInstallationSpecificationDraft } from '../buildInstallationSpecificationDraft';
import type { BuildInstallationSpecificationDraftInput } from '../buildInstallationSpecificationDraft';
...
describe('buildInstallationSpecificationDraft — empty input', () => {
  it('returns a valid draft with both system families as unknown', () => {
    const plan = buildInstallationSpecificationDraft();
...
describe('buildInstallationSpecificationDraft — identifiers', () => {
  it('uses the supplied planId when provided', () => {
    const plan = buildInstallationSpecificationDraft({ planId: 'plan-abc-123' });
    expect(plan.planId).toBe('plan-abc-123');
  });

  it('uses the supplied visitId when provided', () => {
    const plan = buildInstallationSpecificationDraft({ visitId: 'visit-xyz' });
...
describe('buildInstallationSpecificationDraft — currentSystem from survey', () => {
  ...
  it('falls back to unknown when surveyInput is absent', () => {
    const plan = buildInstallationSpecificationDraft({ surveyInput: undefined });
    expect(plan.currentSystem.family).toBe('unknown');
  });
...
describe('buildInstallationSpecificationDraft — proposedSystem from recommendation', () => {
  ...
  it('falls back to unknown when no recommendation is provided', () => {
    const plan = buildInstallationSpecificationDraft({ recommendation: undefined });
    expect(plan.proposedSystem.family).toBe('unknown');
  });
});
...
describe('buildInstallationSpecificationDraft — jobClassification', () => {
...
describe('buildInstallationSpecificationDraft — location import from sessionCapture', () => {
...
describe('buildInstallationSpecificationDraft — route import', () => {
...
describe('buildInstallationSpecificationDraft — flue route import', () => {
...
describe('buildInstallationSpecificationDraft — session without quotePlannerEvidence', () => {
  it('builds a valid plan with empty arrays when quotePlannerEvidence is absent', () => {
    const input: BuildInstallationSpecificationDraftInput = {
      surveyInput:    baseSurveyInput('combi'),
      recommendation: baseScenarioResult('system'),
      sessionCapture: baseCapture(), // no quotePlannerEvidence
    };
    const plan = buildInstallationSpecificationDraft(input);
