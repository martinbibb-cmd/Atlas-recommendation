/**
 * quotePlannerSelectors.test.ts
 *
 * Unit tests for quotePlannerSelectors.
 *
 * Covers:
 ...
 *   - hasQuotePlannerMinimumLocations
 */
...
describe('hasQuotePlannerMinimumLocations', () => {
  it('returns false when plan has no locations', () => {
    expect(hasQuotePlannerMinimumLocations(emptyPlan())).toBe(false);
  });
  ...
  it('returns false when only a boiler location exists (no gas meter)', () => {
    ...
    expect(hasQuotePlannerMinimumLocations(plan)).toBe(false);
  });
  ...
  it('returns true when proposed_boiler and gas_meter are both present', () => {
    ...
    expect(hasQuotePlannerMinimumLocations(plan)).toBe(true);
  });
