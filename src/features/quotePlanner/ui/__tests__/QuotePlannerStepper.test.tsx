/**
 * QuotePlannerStepper.test.tsx
 *
 * Acceptance tests for the Quote Planner visual stepper shell.
 *
...
import { QuotePlannerPage } from '../QuotePlannerPage';

// ─── Helpers ──────────────────────────────────────────────────────────────────

function renderStepper(onBack = vi.fn()) {
  return render(<QuotePlannerStepper onBack={onBack} />);
}

function renderSeededStepper(seedValue: 'combi' | 'system_boiler' | 'heat_pump', onBack = vi.fn()) {
  return render(<QuotePlannerStepper onBack={onBack} seedProposedSystem={seedValue} />);
}

// ─── 1. Renders route / page ──────────────────────────────────────────────────

describe('QuotePlannerStepper — renders', () => {
...
describe('QuotePlannerStepper — current system selection', () => {
...
describe('QuotePlannerStepper — proposed system selection', () => {
...
describe('QuotePlannerStepper — Atlas Pick badge', () => {
...
describe('QuotePlannerStepper — Back/Next navigation', () => {
  ...
  it('progress pill for the active step is labelled "Current" on step 0', () => {
    ...
    expect(activePills[0].textContent).toBe('Current');
  });
  ...
  it('progress pill advances to "Proposed" on step 1', () => {
    ...
    const activePills = document.querySelectorAll('.qp-progress__step--active');
    expect(activePills.length).toBe(1);
    expect(activePills[0].textContent).toBe('Proposed');
  });
});

// ─── QuotePlannerPage — smoke test ───────────────────────────────────────────

describe('QuotePlannerPage', () => {
  it('renders the stepper through the page wrapper', () => {
    render(<QuotePlannerPage onBack={vi.fn()} />);
    expect(screen.getByText('What system do you have now?')).toBeTruthy();
  });

  it('accepts a seed proposed system and passes it to the stepper', () => {
    render(<QuotePlannerPage onBack={vi.fn()} seedProposedSystem="heat_pump" />);
