 ...
 *   - Content derived entirely from AtlasDecisionV1 fields.
...
function buildAiHandoffText(
  ...
  if (decision.keyReasons.length > 0) {
    lines.push('Why selected:');
    decision.keyReasons.slice(0, 3).forEach((r) => lines.push(`• ${r}`));
    lines.push('');
  }

  // Rejected alternatives derived from non-recommended scenarios
  const rejected = scenarios.filter((s) => s.scenarioId !== decision.recommendedScenarioId);
  if (rejected.length > 0) {
    lines.push('Rejected alternatives:');
    rejected.forEach((s) => {
      const constraint = s.keyConstraints[0] ?? 'not suitable for this home';
      lines.push(`• ${s.system.summary} — ${constraint}`);
    });
    lines.push('');
  }

  if (decision.supportingFacts.length > 0) {
    lines.push('Household facts:');
    decision.supportingFacts
      .slice(0, 6)
      .forEach((f) => lines.push(`• ${f.label}: ${f.value}`));
    lines.push('');
  }

  if (decision.includedItems.length > 0) {
    lines.push('Included scope:');
    decision.includedItems.slice(0, 8).forEach((i) => lines.push(`• ${i}`));
    lines.push('');
  }

  if (decision.requiredWorks.length > 0) {
    lines.push('Required works:');
    decision.requiredWorks.slice(0, 5).forEach((w) => lines.push(`• ${w}`));
    lines.push('');
  }

  if (decision.compatibilityWarnings.length > 0) {
    lines.push('Warnings:');
    decision.compatibilityWarnings.slice(0, 3).forEach((w) => lines.push(`• ${w}`));
    lines.push('');
  }

  if (decision.futureUpgradePaths.length > 0) {
    lines.push('Future upgrades:');
    decision.futureUpgradePaths.slice(0, 4).forEach((p) => lines.push(`• ${p}`));
    lines.push('');
  }
