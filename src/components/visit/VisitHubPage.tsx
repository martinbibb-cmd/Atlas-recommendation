...
function buildCustomerSummaryText(meta: VisitMeta, preview: CustomerPreview): string {
  const address = visitDisplayLabel(meta);
  const lines: string[] = [
    'ATLAS VISIT SUMMARY',
    '===================',
    '',
  ...
  if (meta.postcode) lines.push(`Postcode: ${meta.postcode}`);
  if (meta.completed_at) {
    lines.push(
      `Visit completed: ${new Date(meta.completed_at).toLocaleString('en-GB', {
  ...
  lines.push(preview.currentSystem ?? 'Current system details not recorded.');
  lines.push('', "WHAT'S PLANNED", '--------------');
  lines.push(preview.plannedSystem ?? 'Recommendation not yet recorded.');
  lines.push('', 'NEXT STEPS', '----------');
  lines.push('Your engineer will be in touch to confirm the installation date.');
  lines.push(
    '',
    `Generated by Atlas on ${new Date().toLocaleDateString('en-GB', {
      day: 'numeric', month: 'long', year: 'numeric',
    })}`,
  );
  return lines.join('\n');
