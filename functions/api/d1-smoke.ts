 ...
 *
 * The ATLAS_REPORTS_D1 binding is wired in wrangler.jsonc and must be configured in the
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  try {
    const row = await env.ATLAS_REPORTS_D1.prepare("SELECT 1 AS ok").first<{ ok: number }>();
